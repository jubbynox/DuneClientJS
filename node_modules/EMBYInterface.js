  ...
  switch (message.MessageType) {
    case 'Play':
      if (message.Data.PlayCommand === 'PlayNow') {
        ...
        }, function(position, isStopped, itemId) {
          self.sendUpdate(position, isStopped, itemId, self.header, self.hostM);
...
EMBYInterface.prototype.handleServerStart=function() {
  console.log("restarting...");
  var self = this;
  setTimeout(function(){self.launch(self.successCallBack);}, 10000); //relaunch after 10 seconds
}
...
EMBYInterface.prototype.clientInterface = function(err, res, body, accessToken) {
  ...
  client.on('connect', function(connection) {
    ...
    connection.on('close', function() { //likely that the server restarted
      console.log('closed');
      //setTimeout(function() { //restart dune
        //self.launch(self.successCallBack);
      //}, 10000); //wait ten seconds for restart
    });
